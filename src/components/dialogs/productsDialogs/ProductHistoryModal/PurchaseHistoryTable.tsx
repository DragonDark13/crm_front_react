...
const PurchaseHistoryTable: React.FC<PurchaseHistoryTableProps> = ({
    ...
    return (
        <React.Fragment>
            ...
            <CustomDialog title={'Підтвердження видалення'} handleClose={() => setOpenConfirmDialog(false)}
                          open={openConfirmDialog}>

                <DialogContent>
                    <Typography>
                        Ви дійсно хочете видалити запис закупівлі товару від
                        постачальника <strong>{recordToDelete?.supplier.name}</strong> на
                        дату <strong>{new Date(recordToDelete?.purchase_date || '').toLocaleDateString()}</strong> на
                        сумму {recordToDelete?.purchase_total_price} у кількості {recordToDelete?.quantity_purchase}шт?
                    </Typography>
                </DialogContent>
                <DialogActions>
                    <CancelButton text={'Скасувати'} onClick={() => setOpenConfirmDialog(false)}/>
                    <Button
                        onClick={() => {
                            if (recordToDelete) {
                                try {
                                    onDeleteHistoryRecord(recordToDelete.product_id, 'purchase', recordToDelete.id);
                                    refreshHistory(); // <--- оновлення після видалення
                                } catch (error) {
